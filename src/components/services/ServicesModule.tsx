...
export const ServicesModule: React.FC<ServicesModuleProps> = ({
  ...
  // Helper para identificar se OS foi paga
  ...
  const sortedAndFilteredOrders = useMemo(() => {
    return orders
      .filter((os) => {
        ...
        if (hidePaid && isOrderPaid(os)) return false;
  ...
  }, [orders, statusFilter, searchTerm, sortOrder, hidePaid]);
  ...
  const counts = useMemo(() => {
    const map = {
      ...
      OPEN: activeOrdersForCounts.filter((o) => o.status === 'OPEN').length,
      ANALYSIS_BOARD: activeOrdersForCounts.filter((o) => o.status === 'ANALYSIS_BOARD').length,
      WAITING_PARTS: activeOrdersForCounts.filter((o) => o.status === 'WAITING_PARTS').length,
      IN_PROGRESS: activeOrdersForCounts.filter((o) => o.status === 'IN_PROGRESS').length,
      FINISHED_READY: activeOrdersForCounts.filter((o) => o.status === 'FINISHED_READY').length,
      WAITING_PICKUP: activeOrdersForCounts.filter((o) => o.status === 'WAITING_PICKUP').length,
      DELIVERED: activeOrdersForCounts.filter((o) => o.status === 'DELIVERED').length,
    };
    return map;
